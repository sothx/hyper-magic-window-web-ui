      ...
      const [submitUpdateEmbeddedAppErr, submitUpdateEmbeddedAppRes] =
        await $to(
          ksuApi.updateEmbeddedApp({
            ...
            switchAction: {
              name: importRuleContent.name,
              action: ['embedded', 'fullScreen'].includes(
                importRuleContent.mode
              )
                ? 'enable'
                : 'disable',
...
const openAddEmbeddedApp = async () => {
  ...
  if (deviceStore.deviceCharacteristics !== 'tablet') {
    modal.create({
      ...
      content: () => (
        <p>
          该功能仅兼容平板设备，暂时不兼容折叠屏设备，请等待后续更新情况！
        </p>
      ),
    });
    logsStore.info(
  ...
  if (
    ...
    ![32, 33, 34].includes(deviceStore.androidTargetSdk)
  ) {
    modal.create({
      title: '不兼容说明',
      type: 'warning',
      ...
      content: () => (
        <p>
          该功能尚未对 Android 11 或 Android 15+ 做兼容，请等待后续更新情况！
        </p>
      ),
    });
    return;
  ...
  if (addEmbeddedApp.value) {
    const [addEmbeddedAppCancel, addEmbeddedAppRes] = await $to(
      addEmbeddedApp.value.openDrawer()
    );
    if (addEmbeddedAppCancel) {
      ...
      if (addEmbeddedAppRes.settingMode === 'fullScreen') {
        ...
        embeddedStore.customConfigFixedOrientationList[
          ...
          name: addEmbeddedAppRes.name,
          ...(addEmbeddedAppRes.modePayload.isShowDivider
            ? { isShowDivider: true }
            : {}),
          ...(addEmbeddedAppRes.modePayload.skipSelfAdaptive
            ? { disable: true }
            : {}),
          ...(addEmbeddedAppRes.modePayload.supportFullSize
      ...
      if (submitAddEmbeddedAppErr) {
        ...
        addEmbeddedAppRes.loadingCallback &&
          addEmbeddedAppRes.loadingCallback();
      } else {
        modal.create({
          title: '应用添加成功',
...
const openUpdateEmbeddedApp = async (
  row: EmbeddedMergeRuleItem,
  index: number
) => {
  if (updateEmbeddedApp.value) {
    const [updateEmbeddedAppCancel, updateEmbeddedAppRes] = await $to(
      updateEmbeddedApp.value.openDrawer(row)
    );
    if (updateEmbeddedAppCancel) {
      ...
      if (updateEmbeddedAppRes.settingMode === 'fullScreen') {
        ...
        if (embeddedStore.customConfigFixedOrientationList[row.name]) {
          if (
            updateEmbeddedAppRes.modePayload.hasOwnProperty('isShowDivider')
          ) {
            embeddedStore.customConfigFixedOrientationList[
              row.name
            ].isShowDivider = updateEmbeddedAppRes.modePayload.isShowDivider;
          }
          if (
            updateEmbeddedAppRes.modePayload.hasOwnProperty(
              'skipSelfAdaptive'
            )
          ) {
          ...
          if (
            updateEmbeddedAppRes.modePayload.hasOwnProperty('supportFullSize')
          ) {
            embeddedStore.customConfigFixedOrientationList[
              row.name
            ].supportFullSize =
      ...
      if (updateEmbeddedAppRes.settingMode === 'fixedOrientation') {
        if (embeddedStore.customConfigFixedOrientationList[row.name]) {
          ...
          const hasIsScale =
            embeddedStore.customConfigFixedOrientationList[
              row.name
            ].hasOwnProperty('isScale');
          if (hasIsScale) {
            delete embeddedStore.customConfigFixedOrientationList[row.name]
              .isScale;
          }
          if (updateEmbeddedAppRes.modePayload.ratio !== undefined) {
            embeddedStore.customConfigFixedOrientationList[row.name].ratio =
          ...
          if (updateEmbeddedAppRes.modePayload.foRelaunch !== undefined) {
            embeddedStore.customConfigFixedOrientationList[row.name].relaunch =
      ...
      if (submitUpdateEmbeddedAppErr) {
        ...
        updateEmbeddedAppRes.loadingCallback &&
          updateEmbeddedAppRes.loadingCallback();
      } else {
        modal.create({
          title: '应用更新成功',
        ...
        updateEmbeddedAppRes.loadingCallback &&
          updateEmbeddedAppRes.loadingCallback();
        updateEmbeddedAppRes.closeCallback &&
          updateEmbeddedAppRes.closeCallback();
      }
    }
  }
        ...
        return (
          <div>
            {rowIsSupportEmbedded && (
              <n-tag type="success" bordered={false} class="mr-2 my-1">
                平行窗口
              </n-tag>
            )}
            {rowIsSupportFullScreen && (
              <n-tag type="info" bordered={false} class="mr-2 my-1">
                全屏
              </n-tag>
            )}
            {rowIsSupportFixedOrientation && (
              <n-tag type="warning" bordered={false} class="mr-2 my-1">
                居中布局
              </n-tag>
            )}
            ...
            onClick(row: EmbeddedMergeRuleItem, index: number) {
              ...
              if (
                ...
                modal.create({
                  ...
                  content: () => (
                    <p>
                      该功能尚未对 Android 11 或 Android 15+
                      做兼容，请等待后续更新情况！
                    </p>
                  ),
                });
