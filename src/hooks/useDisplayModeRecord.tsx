		...
		modal.create({
			...
			content: () => (
				<div>
					...
					Hz，在设备下次重启前将一直维持每6秒自动轮询应用该配置，该功能可能受其他第三方模块影响不一定生效，如需恢复系统设置内的默认分辨率及刷新率配置，请手动重启设备。
		...
		if (data.id === autoEnableID.value) {
			...
			modal.create({
				...
				content: () => (
					<div>
						应用后设备分辨率将配置为{data.width}x{data.height}，刷新率将配置为{data.fps}
						Hz，每次设备开机后将每6秒自动轮询启动该配置，该功能可能受其他第三方模块影响不一定生效，如需恢复系统设置内的默认分辨率及刷新率配置，请移除该配置的开机自启并手动重启设备。
						{deviceStore.deviceType === 'tablet' && (
							<span>
								请注意小米平板手写笔仅能工作在60hz和120hz，其他刷新率将导致小米平板手写笔无法正常工作，
							</span>
	...
	const setDisplayMode = async (displayModeID: number) => {
		...
		if (setDisplayModeRes) {
			modal.create({
				...
				content: () => (
					<p>
						已成功应用该分辨率及刷新率配置，在设备下次重启前将一直维持每6秒自动轮询应用该配置，如需恢复系统设置内的默认分辨率及刷新率配置，请手动重启设备。
					</p>
				),
				positiveText: '确定',
			});
