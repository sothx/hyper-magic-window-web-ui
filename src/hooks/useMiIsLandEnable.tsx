...
export function useMiIsLandEnable() {
	...
	const currentMiIslandAutoEnableTask = ref<boolean>(false);
	...
	const changeEnableMode = async (mode: boolean) => {
		...
		if (positiveRes) {
			deviceApi
				...
				.then(async res => {
					if (mode) {
						const [addIsMiIsLandAutoEnableTaskErr, addIsMiIsLandAutoEnableTaskRes] = await $to(
							deviceApi.addIsMiIsLandAutoEnableTask(),
						);
						if (addIsMiIsLandAutoEnableTaskErr) {
							modal.create({
								title: '操作失败',
								type: 'error',
								preset: 'dialog',
							...
							currentMiIslandAutoEnableTask.value = mode;
						}
					} else {
						const [removeIsMiIsLandAutoEnableTaskErr, removeIsMiIsLandAutoEnableTaskRes] = await $to(
							deviceApi.removeIsMiIsLandAutoEnableTask(),
						);
						if (removeIsMiIsLandAutoEnableTaskErr) {
							modal.create({
								title: '操作失败',
								type: 'error',
								preset: 'dialog',
