...
async function testEncryptDecryptConsistency() {
  ...
  // 4. 再次解密为字符串
  const decryptedAgain = decryptAesEcb(encryptedBytes.buffer as ArrayBuffer, key);
...
onMounted(async () => {
      ...
      content: () => (
        <div>
          ...
          <p class="whitespace-wrap">
            您也可前往
            <span>/data/adb/modules/Hyper_MagicWindow/common/apks</span>
  ...
  if (deviceStore.moduleInfo?.updateJson) {
    deviceApi.getModuleUpdateMsg(deviceStore.moduleInfo.updateJson).then(res => {
      ...
      // 更新app下载链接
      appStore.syncRemoteDownloadAppUrlMap({
        silent: true
      })
    });
  }
