...
export const saveLogs = (content:string,timestamp:number): Promise<string> => {
	const shellCommon = `echo '${content}' > /data/adb/MIUI_MagicWindow+/logs-${timestamp}.txt && echo "save command executed successfully." || echo "save command failed."`;
	return handlePromiseWithLogging(
		new Promise(async (resolve, reject) => {
			if (import.meta.env.MODE === 'development') {
				...
				errno
					? reject(stderr)
					: stdout === 'save command executed successfully.'
						? resolve(stdout)
						: reject(stdout);
			}
		}),
