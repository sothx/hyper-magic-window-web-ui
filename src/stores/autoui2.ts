    ...
    async function initDefault() {
      ...
      if (getSourceAutoUI2ListErr) {
        sourceAutoUI2List.value = {}
			...
			// 获取自定义配置嵌入规则列表
			const [getCustomConfigAutoUI2ListErr, getCustomConfigAutoUI2ListRes] = await $to(
				autouiApi.getCustomConfigAutoUI2List(),
			);
			if (!getCustomConfigAutoUI2ListErr) {
				customConfigAutoUI2List.value = xmlFormat.parseAutoUI2PackageRulesXml(
					getCustomConfigAutoUI2ListRes
				);
			}
