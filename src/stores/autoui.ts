...
export const useAutoUIStore = defineStore("autoui", () => {
  // 应用布局优化
  const sourceAutoUIList = ref<
    Record<AutoUIItem["name"], AutoUIItem>
  >({});
  ...
  async function initDefault() {
    // 获取源应用布局优化列表
    const [getSourceAutoUIListErr, getSourceAutoUIListRes] =
      await $to(ksuApi.getSourceAutoUIList());
    ...
    // 获取设置配置
    const [getAutoUISettingConfigErr, getAutoUISettingConfigRes] =
      await $to(ksuApi.getAutoUISettingConfig());
    ...
    // 合并最终配置
    mergeRuleList.value = xmlFormat.mergeAutoUIRule(sourceAutoUIList.value,customConfigAutoUIList.value,autoUISettingConfig.value);
