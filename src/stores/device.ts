  ...
  () => {
    ...
    async function initDefault() {
      ...
      if (getRootManagerInfo?.length) {
        ...
        rootManagerInfo.KSU_KERNEL_VER_CODE = parseInt(
          rootManagerStrArr[3],
          10
        );
      ...
      // 旋转建议提示按钮 *强校验
      const [getRotationSuggestionsErr, getRotationSuggestionsRes] = await $to<
        string,
        string
      ...
      if (getRotationSuggestionsErr) {
        ...
        showRotationSuggestions.value =
          Number(getRotationSuggestionsRes) === 1 ? true : false;
      }
      ...
      // 游戏显示布局 *弱校验
      const [, getMiuiCompatEnableRes] = await $to(
        ksuApi.getMiuiCompatEnable()
      );
      if (getMiuiCompatEnableRes && getMiuiCompatEnableRes === 'true') {
        miuiCompatEnable.value = true;
      }
      const [, getMiuiAppCompatEnableRes] = await $to(
        ksuApi.getMiuiAppCompatEnable()
      );
      if (getMiuiAppCompatEnableRes && getMiuiCompatEnableRes === 'true') {
        miuiAppCompatEnable.value = true;
      }
      // Xiaomi Hyper OS 版本号 *弱校验
      const [, getMIOSVersionRes] = await $to<number, string>(
        ksuApi.getMIOSVersion()
      );
      ...
      // 智能IO调度 *弱校验
      const [, getSmartFocusIO] = await $to<
        ksuApi.SmartFocusIOResult['stdout'],
        string
