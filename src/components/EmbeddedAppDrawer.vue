<script setup lang="tsx">
  ...
  const currentFixedOrientationRatio = ref<fixedOrientationRatioOptions>(
    fixedOrientationRatioOptions[1]
  );
  ...
  const embeddedAppDrawer = ref({
    openDrawer: (
      initialParams?: EmbeddedMergeRuleItem
    ): Promise<EmbeddedAppDrawerSubmitResult> => {
      return new Promise((resolve, reject) => {
        ...
        if (
          ...
          reject(
            new Error(
              '该功能暂时未兼容Android 11或Android 15+，请等待后续消息~'
            )
          );
          message.warning(
            '该功能暂时未兼容Android 11或Android 15+，请等待后续消息~'
        ...
        if (props.type === 'update' && initialParams) {
          ...
          if (
            initialParams.embeddedRules &&
            initialParams.embeddedRules.hasOwnProperty('splitRatio')
          ) {
            currentSplitRatio.value =
              initialParams.embeddedRules.splitRatio ?? 0.5;
          }
          const initialSupportModes: EmbeddedMergeRuleItem['settingMode'][] = [
            'disabled',
          ];
          if (
            initialParams.fixedOrientationRule?.supportModes?.includes('fo')
          ) {
            initialSupportModes.unshift('fixedOrientation');
          }
          if (
            initialParams.fixedOrientationRule?.supportModes?.includes('full')
          ) {
          ...
          currentSkipSelfAdaptive.value =
            initialParams.fixedOrientationRule?.disable ?? false;
          currentIsShowDivider.value =
            initialParams.fixedOrientationRule?.isShowDivider ?? false;
          currentFullRule.value =
            initialParams.embeddedRules?.fullRule ?? undefined;
          if (currentFullRule.value === 'nra:cr:rcr:nr') {
            currentFullScreenRuleOptions.value = fullScreenRuleOptions[0];
          } else if (
            initialParams.embeddedRules &&
            !initialParams.embeddedRules.hasOwnProperty('fullRule')
          ) {
            currentFullRule.value = 'nra:cr:rcr:nr';
            currentFullScreenRuleOptions.value = fullScreenRuleOptions[0];
          ...
          currentSupportFullSize.value =
            initialParams.embeddedRules?.supportFullSize ?? false;
          currentRatio.value =
            initialParams.fixedOrientationRule?.ratio ?? undefined;
          if (currentRatio.value) {
            if (currentRatio.value === 1.1) {
              currentFixedOrientationRatio.value =
                fixedOrientationRatioOptions[0];
            } else if (currentRatio.value === 1.5) {
              currentFixedOrientationRatio.value =
                fixedOrientationRatioOptions[2];
            } else if (currentRatio.value === 1.8) {
              currentFixedOrientationRatio.value =
                fixedOrientationRatioOptions[3];
            } else {
              currentFixedOrientationRatio.value =
                fixedOrientationRatioOptions[4];
            }
          } else {
            currentFixedOrientationRatio.value =
              fixedOrientationRatioOptions[1];
  ...
  // 选择全屏规则时更新
  const handleFullScreenRuleSelect = (
    key: string,
    option: fullScreenRuleOptions
  ) => {
    currentFullScreenRuleOptions.value = option;
    currentFullRule.value = [
      'fullScreen_nra:cr:rcr:nr',
      'fullScreen_*',
    ].includes(key)
      ? option.rule
      : undefined;
  ...
  // 选择显示比例时更新
  const handleFixedOrientationRatioSelect = (
    key: string,
    option: fixedOrientationRatioOptions
  ) => {
    currentFixedOrientationRatio.value = option;
    currentRatio.value = ['ratio_11_10', 'ratio_15_10', 'ratio_18:10'].includes(
      key
    )
      ? option.ratio
      : key === 'ratio_custom'
  ...
  const currentSettingMode =
    ref<EmbeddedMergeRuleItem['settingMode']>('fullScreen');
  ...
  const handleDrawerSubmit = async () => {
    ...
    if (
      props.type === 'add' &&
      embeddedStore.allPackageName.has(currentAppName.value)
    ) {
      modal.create({
        title: '应用包名已存在',
        type: 'error',
    ...
    if (
      props.type === 'update' &&
      currentRuleMode.value === 'module' &&
      currentIsSwitchEmbeddedCustom.value
    ) {
  ...
  const handleCurrentSupportModes = (
    value: EmbeddedMergeRuleItem['settingMode'][]
  ) => {
  ...
  <n-drawer v-model:show="activeDrawer" :default-width="500" placement="right">
    <n-drawer-content :title="props.title" closable>
      ...
      <n-tabs
        ...
        v-if="isShowTabs"
        animated
        v-model:value="currentSettingMode"
      >
        <n-tab-pane
          name="embedded"
          tab="平行窗口"
          v-if="props.type === 'update' && isSupportEmbedded"
        >
          <n-alert
            :show-icon="false"
            :bordered="false"
          ...
          >
            ...
            <n-switch
              ...
              v-model:value="currentIsSwitchEmbeddedCustom"
              size="large"
            >
        ...
        >
          ...
          >
            ...
            <n-switch
              :rail-style="railStyle"
              v-model:value="currentSkipSelfAdaptive"
              size="large"
            >
          ...
          <n-card :bordered="false" title="平行窗口滑动条" size="small">
            ...
            <n-switch
              :rail-style="railStyle"
              v-model:value="currentIsShowDivider"
              size="large"
            >
          ...
          >
            ...
            <n-switch
              :rail-style="railStyle"
              v-model:value="currentSupportFullSize"
              size="large"
            >
